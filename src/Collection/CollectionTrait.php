...
trait CollectionTrait
{
    ...
    public function median($path = null)
    {
        $elements = $this;
        if ($path !== null) {
            $elements = $elements->extract($path);
        }
     ...
     * @inheritDoc
     */
    public function append($elements): CollectionInterface
    {
        $list = new AppendIterator();
        $list->append($this->unwrap());
        $list->append($this->newCollection($elements)->unwrap());
     ...
     * @inheritDoc
     */
    public function appendItem($element, $key = null): CollectionInterface
    {
        if ($key !== null) {
            $data = [$key => $element];
        } else {
     ...
     * @inheritDoc
     */
    public function prepend($elements): CollectionInterface
    {
        return $this->newCollection($elements)->append($this);
    }

    /**
     * @inheritDoc
     */
    public function prependItem($element, $key = null): CollectionInterface
    {
        if ($key !== null) {
            $data = [$key => $element];
        } else {
     ...
     * @inheritDoc
     */
    public function zip(iterable $elements): CollectionInterface
    {
     ...
     * @inheritDoc
     */
    public function zipWith(iterable $elements, $callback): CollectionInterface
    {
        if (func_num_args() > 2) {
            $elements = func_get_args();
            $callback = array_pop($elements);
        } else {
            $elements = [$elements];
        }

        return new ZipIterator(array_merge([$this->unwrap()], $elements), $callback);
    }
